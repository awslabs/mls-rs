...
use mls_rs_codec::{self, MlsEncode};
use mls_rs_core::{
...
async fn interop_passive_client() {
        ...
        .chain(test_cases_rand)
    {
        ...
        let key_package_data = KeyPackageData::new(
            key_package.mls_encode_to_vec().unwrap(),
            test_case.init_priv.into(),
            test_case.encryption_priv.into(),
            123,
        );
        ...
        let (mut group, _info) = joiner.join().await.unwrap();
        ...
        for epoch in test_case.epochs {
            ...
            processor = test_case
                .external_psks
                .iter()
                .fold(processor, |processor, psk| {
                    processor.with_external_psk(psk.psk_id.clone().into(), psk.psk.clone().into())
                });

            for resumption_psk in processor
                .required_resumption_psk()
                .cloned()
                .collect::<Vec<_>>()
                .iter()
            {
                processor = processor.with_resumption_psk(
                    resumption_psk.clone(),
                    group_clone
                        .resumption_secret(resumption_psk.psk_epoch)
                        .await
                        .unwrap(),
                );
            }
...
#[cfg_attr(coverage_nightly, coverage(off))]
async fn invite_passive_client<P: CipherSuiteProvider, C: MlsConfig>(
    groups: &mut [Group<C>],
    with_psk: bool,
    cs: &P,
    path_required: bool,
    ...
    let client = TestClient::new(client);
    ...
    TestCase {
        ...
        key_package: key_pckg.key_package_data.key_package_bytes,
        encryption_priv: key_pckg.key_package_data.leaf_node_key.to_vec(),
        init_priv: key_pckg.key_package_data.init_key.to_vec(),
...
pub async fn generate_passive_client_proposal_tests() -> Vec<TestCase> {
    ...
    for cs in CipherSuite::all() {
        ...
        let mut groups =
            get_test_groups(VERSION, cs.cipher_suite(), 7, false, &crypto_provider).await;
        ...
        let resumption_psk = groups[1].resumption_secret(epoch_to_resume).await.unwrap();
        ...
        let resumption_psk = groups[4].resumption_secret(epoch_to_resume).await.unwrap();
...
#[cfg_attr(not(mls_build_async), maybe_async::must_be_sync)]
#[cfg_attr(coverage_nightly, coverage(off))]
async fn commit_by_value<F, C: ClientConfig>(
    group: &mut Group<C>,
    proposal_adder: F,
...
async fn create_key_package(cs: CipherSuite) -> MlsMessage {
    let client =
        generate_basic_client(cs, VERSION, 0xbeef, false, &TestCryptoProvider::new()).await;
...
pub async fn generate_passive_client_welcome_tests() -> Vec<TestCase> {
    ...
    for cs in CipherSuite::all() {
        ...
        for with_tree_in_ext in [true, false] {
            ...
            {
                let mut groups =
                    get_test_groups(VERSION, cs.cipher_suite(), 16, false, &crypto_provider).await;
...
pub async fn generate_passive_client_random_tests() -> Vec<TestCase> {
    ...
    for cs in CipherSuite::all() {
        ...
        let creator = generate_basic_client(cs, VERSION, 0, false, &crypto).await;
        ...
        for i in 0..10 {
            new_clients.push(generate_basic_client(cs, VERSION, i + 1, false, &crypto).await)
        }
        ...
        for _ in 0..100 {
            ...
            for i in 0..num_added {
                new_clients.push(
                    generate_basic_client(cs, VERSION, next_free_idx + i, false, &crypto).await,
                );
            }
