...
pub(crate) trait MessageProcessor: Send + Sync {
    ...
    async fn process_incoming_message_with_time(
        ...
        let unauthn_key_gen_in_app_msg: Option<u32> = match message.payload {
            MlsMessagePayload::Cipher(ref cipher_text) => {
                match self.get_unauthenticated_key_generation_from_sender_data(&cipher_text) {
                    Ok(g) => g,
                    Err(..) => None,
                }
            }
    ...
    async fn process_event_or_content(
        ...
        #[cfg(feature = "by_ref_proposal")] cache_proposal: bool,
        #[cfg(feature = "export_key_generation")] unauthn_key_gen_in_app_msg: Option<u32>,
        time_sent: Option<MlsTime>,
    ) -> Result<Self::OutputType, MlsError> {
    ...
    fn process_application_message(
        ...
        #[cfg(feature = "export_key_generation")] unauthenticated_key_generation: Option<u32>,
