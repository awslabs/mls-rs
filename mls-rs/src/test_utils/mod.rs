...
impl<C: MlsConfig> TestClient<C> {
    ...
    pub async fn generate_key_package(&self) -> Result<MlsMessage, MlsError> {
        let package = self.client.key_package_builder(None)?.build().await?;
    ...
    #[cfg_attr(not(mls_build_async), maybe_async::must_be_sync)]
    pub async fn generate_key_package_custom(
        &self,
...
impl<C: MlsConfig> From<Client<C>> for TestClient<C> {
    fn from(value: Client<C>) -> Self {
        Self::new(value)
    }
}
