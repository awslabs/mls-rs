...
fn generate_recipient_key_package(cipher_suite: CipherSuite) -> KeyPackageGeneration {
    ...
    client.gen_key_package(&mut OpenSslRng, &cipher_suite).unwrap()
}

fn create_group(create_inputs: &CreateInputs) -> (Group, Welcome) {
    let mut rng = OpenSslRng;
    let mut test_group = Group::new(&mut rng, b"group".to_vec(),
                                    create_inputs.creator.clone()).unwrap();

    let add_members = test_group.add_member_proposals(&create_inputs.members)
        .unwrap();

    let commit = test_group.commit_proposals(add_members,
                                             true,
                                             &mut rng,
                                             &create_inputs.client).unwrap();
...
fn update_path(client: &Client, group: &mut Group) -> MLSPlaintext {
    let commit = group.commit_proposals(vec![],
                                        true,
                                        &mut OpenSslRng,
                                        client).unwrap();
...
fn receive_group(inputs: ReceiveInputs) -> Group {
    Group::from_welcome_message(inputs.welcome,
                                inputs.ratchet_tree,
                                inputs.key_package).unwrap()
}
...
fn main() {
    ...
    let alice_key_package = alice.gen_key_package(&mut OpenSslRng,
                                                  &cipher_suite).unwrap();

    let bob_key_package = bob.gen_key_package(&mut OpenSslRng,
                                              &cipher_suite).unwrap();

    let charlie_key_package = charlie.gen_key_package(&mut OpenSslRng,
                                                      &cipher_suite).unwrap();
    ...
    println!("CREATE: {:?} time: {}", alice_group.cipher_suite, start_create.elapsed().as_millis());
    ...
    alice_group.process_plaintext(bob_path_update.clone()).unwrap();
