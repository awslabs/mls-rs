...
use aws_mls::credential::{Credential, CREDENTIAL_TYPE_BASIC, CREDENTIAL_TYPE_X509};
use aws_mls::extension::{Extension, ExtensionList, MlsExtension, RequiredCapabilitiesExt};
...
#[test]
fn unsupported_extension_is_filtered() {
    // Introducing an extension that is not supported by a leaf should fail.
    ...
    let ext = ExtensionList::from(vec![Extension {
        extension_type: 42u16,
        extension_data: vec![],
    }]);

    let proposal = groups[0].group_context_extensions_proposal(ext);
    groups[0].proposal_message(proposal, Vec::new()).unwrap();
    groups[0].commit_proposals(vec![], vec![]).unwrap();
    let state_update = groups[0].process_pending_commit().unwrap();
    assert_eq!(state_update.rejected_proposals.len(), 1);
}
...
fn removing_requirements_allows_to_add() {
    ...
    // Require both credential types
    let req_caps = RequiredCapabilitiesExt {
        credentials: vec![CREDENTIAL_TYPE_BASIC, CREDENTIAL_TYPE_X509],
        ..RequiredCapabilitiesExt::default()
    };

    let ext = ExtensionList::from(vec![req_caps.to_extension().unwrap()]);
    ...
    // Create a client who supports only basic credentials
    let key = CipherSuite::Curve25519Aes128
    ...
    let client = InMemoryClientConfig::default()
        ...
        .with_lifetime_duration(ONE_YEAR_IN_SECONDS)
        .with_credential_types(vec![CREDENTIAL_TYPE_BASIC])
        .build_client();
    ...
    // The client can be added if another proposal removes the requirement from capabilities
    let proposal = groups[0].add_proposal(kp).unwrap();
    groups[0].proposal_message(proposal, Vec::new()).unwrap();

    // Remove all extensions, including required capabilities
    let proposal = groups[0].group_context_extensions_proposal(ExtensionList::new());
    groups[0].proposal_message(proposal, Vec::new()).unwrap();

    groups[0].commit_proposals(vec![], vec![]).unwrap();
    let state_update = groups[0].process_pending_commit().unwrap();

    assert_eq!(state_update.rejected_proposals, Vec::new());
}

#[test]
fn cannot_add_leaves_who_dont_support_all_credentials() {
    let protocol_version = ProtocolVersion::Mls10;
    let cipher_suite = CipherSuite::Curve25519Aes128;

    // Create a leaf who supports only basic credentials
    let key = cipher_suite.generate_signing_key().unwrap();
    ...
    let mut group = client
        .create_group(protocol_version, cipher_suite, ExtensionList::new())
        .unwrap();

    // Create a leaf who only supports X509
    let key = cipher_suite.generate_signing_key().unwrap();
    ...
    let client = InMemoryClientConfig::default()
        ...
        .with_preferences(Preferences::default())
        .with_lifetime_duration(ONE_YEAR_IN_SECONDS)
        .with_credential_types(vec![CREDENTIAL_TYPE_X509])
        .build_client();
    ...
    // Add should be filtered
    let proposal = group.add_proposal(kp).unwrap();
    group.proposal_message(proposal, Vec::new()).unwrap();
    group.commit_proposals(vec![], vec![]).unwrap();
    let state_update = group.process_pending_commit().unwrap();
    println!("{:?}", group);
