...
use mls::key_package::{KeyPackageGeneration, KeyPackage, KeyPackageGenerator};
...
fn create_group(create_inputs: &CreateInputs) -> (Group, Welcome) {
    ...
    let mut test_group = Group::new(&mut rng, b"group".to_vec(), create_inputs.creator.clone()).unwrap();
    let add_members = test_group.add_member_proposals(&create_inputs.members).unwrap();
    let commit = test_group.commit_proposals(add_members, true, &mut rng, &create_inputs.client).unwrap();
...
fn receive_group_benchmark(c: &mut Criterion) {
    ...
    let mut recipients = (0..499).into_iter().map(|_| generate_recipient_key_package(cipher_suite.clone()).key_package).collect::<Vec<KeyPackage>>();
