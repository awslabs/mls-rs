...
fn verify_signature_tests<C: CipherSuiteProvider>(cs: &C, test_cases: Vec<SignatureTestCase>) {
    // Checks that `cs` can sign and verify
    let generated = generate_signature_tests(cs);

    for test_case in test_cases.into_iter().chain(generated.into_iter()) {
        ...
        let derived = cs
            .signature_key_derive_public(&test_case.secret.into())
            .unwrap();
...
fn generate_signature_tests<C: CipherSuiteProvider>(cs: &C) -> Vec<SignatureTestCase> {
    DATA_SIZES
        .iter()
        .copied()
        .map(|data_size| {
            ...
            let signature = cs.sign(&secret, &data).unwrap();
