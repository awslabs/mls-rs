...
pub trait Hpke: Sized {
    ...
    fn key_schedule(mode: Mode, shared_secret: &[u8], info: &[u8],
                    psk: Option<&[u8]>, psk_id: Option<&[u8]>) -> Result<KeySchedule, HPKEError> {
    ...
    fn setup_basic_receiver(enc: &[u8],
                            local_secret: &<<Self::KEM as Kem>::E as AsymmetricKeyEngine>::SK,
                            info: &[u8]) -> Result<Context<Self::CT>, HPKEError> {
...
#[cfg(test)]
mod tests {
    use crate::hpke::{Hpke, Mode, P256HkdfSha256Aes128Gcm, P521HkdfSha512Aes256Gcm, X25519HkdfSha256Aes128Gcm, X25519HkdfSha256ChaCha20, X448HkdfSha512Aes256Gcm, X448HkdfSha512ChaCha20};
    ...
    macro_rules! assert_eq_hpke {
        ($a:expr, $b:expr, $tv:expr) => {
            assert_eq!($a,$b, "failing vector: Mode: {:?} KEM: {:?} KDF: {:?} AEAD: {:?}",
                $tv.mode,
                $tv.kem_id,
                $tv.kdf_id,
                $tv.aead_id)
    ...
    fn setup_receiver<OUT: Hpke>(enc: &Vec<u8>, sk_rm: &Vec<u8>, info: &Vec<u8>) -> super::Context<OUT::CT> {
        let local_secret = <<OUT::KEM as Kem>::E as AsymmetricKeyEngine>::SK::from_bytes(sk_rm).unwrap();
    ...
    fn test_hpke<T: Hpke>(vector: &TestVector) {
        ...
        assert_eq_hpke!(s_context.key_schedule.exporter_secret, *vector.exporter_secret, vector);
        assert_eq_hpke!(s_context.key_schedule.base_nonce, *vector.base_nonce, vector);
    ...
    fn test_hpke_vectors() {
        ...
        let test_vectors: Vec<TestVector> = serde_json::from_reader(BufReader::new(file))
