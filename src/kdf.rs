...
mod test {
    ...
    fn run_test_case<KDF: Kdf>(case: TestCase) {
        // Extract phase
        let extracted = KDF::extract(&case.salt, &case.ikm).expect("HKDF extract failed");

        assert_eq!(extracted, case.prk);

        // Expand phase
        let expanded = KDF::expand(&case.prk, &case.info, case.len).expect("HKDF expand failed");
    ...
    fn test_basic_sha256() {
        ...
        let case = TestCase {
            ...
            len: 42,
            prk: hex!(
                "077709362c2e32df0ddc3f0dc47bb\
                       a6390b6c73bb50f9c3122ec844ad7c2b3e5"
            ),
            okm: hex!(
                "3cb25f25faacd57a90434f64d0362f\
