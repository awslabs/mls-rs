    ...
    macro_rules! openssl_asym_key {
        ($nid:expr, $n_sk:expr, $n_pk:expr, $bitmask:expr) => {
            ...
            impl EcdhEngine for Engine {
                fn shared_secret(local_sk: &Self::SK, remote_pk: &Self::PK) -> Result<Vec<u8>, AsymmetricKeyError> {
                    super::ossl::shared_secret(&local_sk.key, &remote_pk.key)
                        .map_err(|e| e.into())
                }
            }
        }
...
pub mod p521 {
    ...
    mod test {
        ...
        fn get_test_case() -> TestCase {
            TestCase {
                alice_pub: hex!("0401ebb34dd75721abf8adc9dbed178\
...
pub mod p256 {
    ...
    mod test {
        ...
        fn get_test_case() -> TestCase {
            TestCase {
                alice_pub: hex!("042af502f3be8952f2c9b5a8d4160d09e97165be50bc4\
                                 2ae4a5e8d3b4ba83aeb15eb0faf4ca986c4d38681a0f9\
                                 872d79d56795bd4bff6e6de3c0f5015ece5efd85"),
                alice_pri: hex!("814264145f2f56f2e96a8e337a1284993faf432a5abce\
                                 59e867b7291d507a3af"),
                bob_pub: hex!("04b120de4aa36492795346e8de6c2c8646ae06aaea279fa77\
                               5b3ab0715f6ce51b09f1b7eece20d7b5ed8ec685fa3f071\
                               d83727027092a8411385c34dde5708b2b6"),
                bob_pri: hex!("2ce1788ec197e096db95a200cc0ab26a19ce6bccad562b8\
                               eee1b593761cf7f41"),
                shared_secret: hex!("dd0f5396219d1ea393310412d19a08f1f5811e9dc\
...
pub mod x25519 {
    ...
    impl EcdhEngine for Engine {
        fn shared_secret(local_sk: &Self::SK, remote_pk: &Self::PK) -> Result<Vec<u8>, AsymmetricKeyError> {
            let shared_secret = local_sk.key
    ...
    mod test {
        ...
        fn get_test_case() -> TestCase {
            TestCase {
                alice_pub: hex!("8520f0098930a754748b7ddcb43ef75\
                                 a0dbf3a0d26381af4eba4a98eaa9b4e6a"),
                alice_pri: hex!("70076d0a7318a57d3c16c17251b2664\
                                 5df4c2f87ebc0992ab177fba51db92c6a"),
                bob_pub: hex!("de9edb7d7b7dc1b4d35b61c2ece435373\
                               f8343c85b78674dadfc7e146f882b4f"),
                bob_pri: hex!("5dab087e624a8a4b79e17f8b83800ee66\
                               f3bb1292618b6fd1c2f8b27ff88e0eb"),
                shared_secret: hex!("4a5d9d5ba4ce2de1728e3bf4803\
...
pub mod x448 {
    ...
    impl EcdhEngine for Engine {
        fn shared_secret(local_sk: &Self::SK, remote_pk: &Self::PK) -> Result<Vec<u8>, AsymmetricKeyError> {
            if let Some(shared_secret) = local_sk.key
                .as_diffie_hellman(&remote_pk.key) {
...
mod test {
    ...
    pub fn run_ecdh_test_case<
        PK: PublicKey,
        SK: SecretKey,
        E: EcdhEngine<PK = PK, SK = SK>
    >(case: TestCase) {
        ...
        let shared_secret_a = E::shared_secret(&alice_pri, &bob_pub)
            .expect("shared secret generation failed");

        // Use the ecdh engine to compute the shared secret (bob)
        let shared_secret_b = E::shared_secret(&bob_pri, &alice_pub)
            .expect("shared secret generation failed");
    ...
    >(case: TestCase) {
        ...
        assert_eq!(pub_key.to_bytes().expect("key serialize failed"), case.alice_pub);
    ...
    >(case: TestCase) {
        ...
        let rng = test_rng::OneValRng { val: case.alice_pri.clone() };

        // Generate a random keypair with our fixed rng
        let (pk, sk) = E::random_key_pair(rng)
